/**
 * SkydiveLog · Typography for React Native
 * Every font size used across the designs. Pair with fonts from expo-font:
...
/** Numeric weight aliases (occasionally easier to read in JSX). */
...
/**
 * Type scale — RN style objects ready to spread into Text.
 * `fontFamily` defaults to UI; mono variants are suffixed `*Mono`.
 */
type TypeStyle = {
  fontFamily: string;
  fontSize: number;

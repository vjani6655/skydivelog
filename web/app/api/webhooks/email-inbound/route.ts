...
// Resend sends inbound email payloads as JSON POST requests.
...
/** Strip quoted reply content from plain-text email bodies.
 *  Removes everything from the first "On ... wrote:" line or "---" divider downwards.
 */
function stripQuotedText(raw: string): string {
  const lines = raw.split('\n')
  const cutPatterns = [
    /^On .+wrote:\s*$/,           // "On Mon, 28 May ... John wrote:"
    /^>.*/,                        // "> quoted line"
    /^-{3,}/,                      // "--- Original message ---"
    /^_{3,}/,                      // "___ separator ___"
    /^From:\s/i,                   // forwarded header
    /^Sent:\s/i,
    /^To:\s/i,
    /^Subject:\s/i,
  ]
